...
class ColorLayer(Paidiverpy):
    ...
    def run(self):
        ...
        for index, img in tqdm(
            ...
            if method_name == "edge_detection":
                img_data, features = method(img_data, test=test, params=params)
            else:
                img_data = method(img_data, test=test, params=params)
    ...
    def grayscale(self, img, test=False, params: GrayScaleParams = GrayScaleParams()):
        if not test:
            try:
    ...
    def gaussian_blur(
        self, img, test=False, params: GaussianBlurParams = GaussianBlurParams()
    ):
        if not test:
            try:
    ...
    def sharpen(self, img, test=False, params: SharpenParams = SharpenParams()):
        if not test:
            try:
    ...
    def edge_detection(
        self, img, test=False, params: EdgeDetectionParams = EdgeDetectionParams()
    ):
        if not test:
            try:
