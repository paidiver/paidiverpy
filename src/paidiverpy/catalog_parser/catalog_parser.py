...
class CatalogParser:
    def __init__(self,
        ...
        self.config = config or self._build_config(catalog_path, catalog_type, append_data_to_catalog)
        self.catalog_type = getattr(self.config.general, 'catalog_type', None)
        self.append_data_to_catalog = getattr(self.config.general, 'append_data_to_catalog', None)
    ...
    def _process_coordinates(self, catalog):
        ...
        if 'lon' in catalog.columns and 'lat' in catalog.columns:
            catalog['point'] = catalog.apply(lambda x: Point(x['lon'], x['lat']), axis=1)
    ...
    def _rename_columns(self, catalog, columns, raise_error=False):
        if not any(col in catalog.columns for col in columns):
            if raise_error:
                self.logger.error("Catalog does not have a %s type column. It should have one of the following columns: %s",  columns[0], columns)
                raise ValueError(f"Catalog does not have a {columns[0]} type column. It should have one of the following columns: {columns}")
            self.logger.warning("Catalog does not have a %s type column. It should have one of the following columns: %s",  columns[0], columns)
    ...
    def _add_data_to_catalog(self, catalog):
        new_catalog = pd.read_csv(self.append_data_to_catalog).drop_duplicates(subset='filename', keep='first')
